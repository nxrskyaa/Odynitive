...
export function LaunchPage() {
  ...
  return <section className="shell launch-page">
    <div className="page-intro"><span className="kicker">A NEW BEGINNING</span><h1>Launch something<br/><em>worth finding.</em></h1><p>One billion fixed tokens. A live market. No presale maze.</p></div>
    <div className="launch-layout">
      ...
      <aside className="launch-preview"><span className="kicker">LIVE PREVIEW</span><div className="preview-token panel"><div className="preview-image">{form.imageURI ? <img src={form.imageURI} alt=""/> : <span>{form.symbol.slice(0,2) || 'O'}</span>}</div><div className="token-symbol">{form.symbol.toUpperCase() || 'TOKEN'}</div><h2>{form.name || 'Untitled token'}</h2><p>{form.description || 'A short, concrete description will make this easier to discover.'}</p><div className="preview-line"><span>Starting price</span><b>0.00001 RITUAL</b></div><div className="preview-line"><span>Network</span><b>Ritual · 1979</b></div></div><Link to="/" className="text-link">← Back to the agora</Link></aside>
    </div>
  </section>
}
