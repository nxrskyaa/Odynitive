...
export function DocsPage() {
  ...
  return <section className="shell info-page">
    ...
    <BilingualSection language={lang}
      id={<><p className="lead">Odynitive adalah launchpad dan pasar token berbasis bonding curve di Ritual Testnet. Token dapat dibuat tanpa coding, lalu langsung dibeli dan dijual melalui kontrak yang sama. Ini adalah satu dari empat product realm—bersama NFTs, Agentz, dan realm GameFi Odyvion.</p><div className="notice"><b>Status saat ini: testnet</b><span>Token dan RITUAL di sini tidak mewakili aset mainnet. Sistem masih dalam tahap pengujian.</span></div></>}
      en={<><p className="lead">Odynitive is a bonding-curve token launchpad and marketplace on Ritual Testnet. Tokens can be created without code, then bought and sold immediately through the same contract. It is one of four product realms—alongside NFTs, Agentz, and the Odyvion GameFi realm.</p><div className="notice"><b>Current status: testnet</b><span>Tokens and RITUAL shown here do not represent mainnet assets. The system is still being tested.</span></div></>}/>
